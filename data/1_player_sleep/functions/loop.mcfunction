...
execute as @a[tag=is_sleeping,scores={sleep_timer=1}] if entity @a[tag=!is_sleeping] run tellraw @a[tag=!is_sleeping] [{"selector":"@s"},{"text":" is now sleeping","color":"gray","clickEvent":{"action":"run_command","value":"/trigger kick_sleeping set 1"},"hoverEvent":{"action":"show_text","value":{"text":"","extra":[{"text":"Kick player out of bed!","color":"aqua"}]}}}]
